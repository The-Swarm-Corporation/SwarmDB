...
class SwarmsDB:
    ...
    def __init__(
        ...
        self.save_dir = (
            Path(save_dir)
            if save_dir
            else Path(os.getcwd()) / "message_history"
        )
        ...
        logger.info(
            f"SwarmsDB initialized with base topic: {base_topic}"
        )
    ...
    def _count_tokens(
        self, content: Union[str, Dict[str, Any], List[Any]]
    ) -> int:
    ...
    def register_agent(self, agent_id: str) -> None:
        ...
        logger.info(
            f"Agent {agent_id} registered with messaging system"
        )
    ...
    def deregister_agent(self, agent_id: str) -> None:
        ...
        logger.info(
            f"Agent {agent_id} deregistered from messaging system"
        )
    ...
    def _delivery_callback(self, err, msg) -> None:
        ...
        if err is not None:
            ...
            if message_id in self.messages:
                self.messages[message_id].status = (
    ...
    def send_message(
        ...
        try:
            ...
            if self.auto_save and (
                ...
                or self.message_count % self.max_messages_per_file
                == 0
            ):
            ...
            try:
                ...
                logger.error(
                    f"Failed to send message to error topic: {nested_e}"
                )
    ...
    def receive_messages(
        ...
        if agent_id not in self.registered_agents:
            logger.warning(
                f"Agent {agent_id} not registered, registering now"
            )
        ...
        while (
            ...
            msg = consumer.poll(
                timeout=self.config.consumer_timeout_ms / 1000
            )
            ...
            try:
                ...
                if (
                    ...
                    agent_id in message.visible_to
                    or len(message.visible_to) == 0
                ):
                    # Update message status
                    ...
                    logger.debug(
                        f"Message {message.id} received by agent {agent_id}"
                    )
    ...
    def load_message_history(
        ...
        if not filepath.exists():
            logger.error(
                f"Message history file {filepath} does not exist"
            )
            return
    ...
    def export_as_yaml(self, filepath: Union[str, Path]) -> None:
        ...
        try:
            ...
            logger.info(
                f"Message history exported as YAML to {filepath}"
            )

        except Exception as e:
            logger.error(
                f"Failed to export message history as YAML: {e}"
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        message_types = {
            msg_type.value: 0 for msg_type in MessageType
        }
        ...
        message_statuses = {
            status.value: 0 for status in MessageStatus
        }
        for msg in self.messages.values():
    ...
    def get_unread_message_count(self, agent_id: str) -> int:
        ...
        return len(
            [
                msg_id
                for msg_id in self.agent_inbox[agent_id]
                if msg_id in self.messages
                and self.messages[msg_id].status
                == MessageStatus.DELIVERED
            ]
        )
    ...
    def get_agent_load(self, agent_id: str) -> Dict[str, Any]:
        ...
        recent_messages = len(
            [
                ...
                for msg in self.messages.values()
                if msg.receiver_id == agent_id
                and msg.timestamp > one_minute_ago
            ]
        )
        ...
        return {
            ...
            "processing_rate": recent_messages
    ...
    def delete_message(self, message_id: str) -> bool:
        ...
        if message_id not in self.messages:
            logger.warning(
                f"Message {message_id} not found for deletion"
            )
            return False
    ...
    def flush_old_messages(
        self, older_than: Optional[float] = None
    ) -> int:
        """
        ...
        if old_message_ids:
            ...
            archive_path = (
                self.save_dir / "archives" / archive_filename
            )
            ...
            logger.info(
                f"Flushed {len(old_message_ids)} old messages to {archive_path}"
            )
    ...
    def add_agent_group(
        ...
        self.metadata["agent_groups"] = self.metadata.get(
            "agent_groups", {}
        )
        self.metadata["agent_groups"][group_name] = agent_ids

        logger.info(
            f"Agent group '{group_name}' created with {len(agent_ids)} agents"
        )
    ...
    def set_llm_load_balancing(self, enabled: bool = True) -> None:
        ...
        logger.info(
            f"LLM load balancing {'enabled' if enabled else 'disabled'}"
        )

    def assign_llm_backend(
        self, agent_id: str, backend_id: str
    ) -> None:
        """
        ...
        """
        self.metadata = getattr(self, "metadata", {})
        self.metadata["llm_backends"] = self.metadata.get(
            "llm_backends", {}
        )
        self.metadata["llm_backends"][agent_id] = backend_id

        logger.info(
            f"Agent {agent_id} assigned to LLM backend {backend_id}"
        )
    ...
    def auto_scale_partitions(self) -> None:
        ...
        if recommended_partitions > current_partitions:
            ...
            try:
                result = self.admin_client.create_partitions(
                    new_partitions_request
                )
                ...
                for topic, future in result.items():
                    try:
                        future.result()
                        logger.info(
                            f"Increased partitions for {topic} to {recommended_partitions}"
                        )
                    except Exception as e:
                        logger.error(
    ...
    def close(self) -> None:
        ...
        for agent_id, consumer in self.consumers.items():
            try:
                ...
                logger.error(
                    f"Error closing consumer for agent {agent_id}: {e}"
                )
